...
\section{Перевод статьи P.Schafer <<Fast and Accurate Time Series Classification with WEASEL>>}

\url{https://arxiv.org/abs/1701.07681}

Подобно другим современным методам классификации временных рядов, WEASEL с помощью скользящего окна преобразует временные ряды в векторы признаков, которые затем анализируются классификатором.

Традиционно подходы к классификации временных рядов делали акцент на точности, но не масштабируемости. Что делает эти методы бесполезными для анализа длинных временных рядов, с которыми обычно приходится иметь дело.

Профили энергопотребления различных устройств различаются заметно, но все же похожи у устройств одного типа. Постановка классификации временных рядов выглядит так: определить тип устройства по профилю энергопотребления.

Современные методы не обладают требуемой скоростью и точностью. Несколько высокоточных классификаторов, таких как Shapelet Transform имеют би-квадратичную временную сложность ($ O(n^4) $) от длины входа. Даже методы с квадртаичной временной сложностью неосуществимы.

Наиболее точный метод COTE состоит из десятков ключевых классификаторов, многие из которых имеют квадратическую, кубическую или би-квадратическую временную сложность. С другой стороны быстрые методы классификации, такие как BOSS VS или Fast Shapelets, работают очень плохо.

Методы на базе евклидового расстояния быстрые, но с низкой точностью. DTW-методы (Dynamic Time Warping) быстрые, но грубые. Высокоточные методы классификаторы такие как, ST и BOSS требуют много времени на построение прогноза.

В статье предлагается новый метод классификации WEASEL (Word ExtraAction for time SEries cLassifiaction). WEASEL и очень быстрый, и точный. Как и некоторые другие методы, WEASEL концептуально основан на так называемом подходе <<мешок паттернов>>. Он скользит окном по временному ряду и извлекает признаки на окне, которые затем предаются в классификатор.

WEASEL использует окна различной длины, а также учитывает порядок расположения окон вместо того, чтобы рассматривать каждое окно фиксированной длины как независимый признак, что позволяет WEASEL лучше улавливать характеристики каждого класса. Кроме того, WEASEL применяет агрессивный статистический отбор признаков вместо простого использования всех признаков для классификации; это приводит  к значительно меньшему пространству признаков и значительному сокращению времени выполнения без ущерба для точности.

В результате набор признаков получается высокодискриминативным, что позволяет использовать быструю логистическую регрессию.

Подходы к задаче классификации временных рядов можно разделить на две группы:
\begin{itemize}
	\item методы меры подобия на базе полных рядов (whole series-based similarity methods),
	
	\item методы на базе признаков (feature-based methods).
\end{itemize}

\emph{Методы меры подобия на базе полных рядов} используют точечное сравнение. К ним относятся евклидово расстояние 1-NN или 1-NN динамическое искажение по времени (DTW), которые обычно используют в качестве базы для сравнения. Как правило, эти подходы хорошо работают для коротких временных рядов, но неэффективны для зашумленных или длинных временных рядов. К тому же у DTW квадратическая $ O(n^2) $ временная сложность для временных рядов длины $ n $. Такие техники как ранняя подрезка кандидатов с каскадными нижними границами может использоваться для сокращения времени выполнения.

А классификаторы, основанные на признаках, полагаются на сравнение признаков, сгенерированных из подструктур временного ряда.

Шеплеты (shepletes) дорого обходятся с вычислительной точки зрения как на этапе обучения, так и на этапе прогноза.

Альтернативным подходом в группе классификаторов, основанных на признаках, является мешок паттернов (bag-of-patterns, BOP). Такие методы разбивают временной ряд на мешок подструктур и представляют эти подструктуры в виде дискретных признаков и, наконец, строят гистограмму признаков в качестве основы для классификации.

Первая опубликованная модель BOP (BOP-SAX) использует скользящие окна фиксированной длины и преобразует измерения в каждом окне в дискретные признаки с помощью символьной аггрегированной аппроксимации (Symbolic Aggregate approXimation, SAX). Классификация реализуется как 1-NN классификатор, использующий евклидово расстояние для подсчета признаков в качестве меры расстояния.

SAX-VSM расширяет BOP-SAX техникой tf-idf-взвешивания признаков и косинусного расстояния. Он создает один вектор признаков на класс вместо одного вектора на экземпляр, что значительно снижает временные издержки на выполнение.

В отличие от подходов, основанных на шеплетах, BOP-подходы имеют линейную временную сложность для классификации.

Наиболее точные алгоритмы классификации временных рядов -- это ансамбли. Задача классификации решается набором различных классификаторов с последующей агрегацией результатов.

Ансамбль эластичных классификаторов (Elastic Ensemble, EE PROP)  используется 11 классификаторов на базе полного ряда, включая DTW CV, DTW, LCSS и ED. Ансамбль COTE основывается на 35 базовых методах, включая EE PROP и ST.

Размерность BOP пространства признаков -- $ O(c^l) $ для слова длины $ l $ и $ c $ символов. Это не зависит от числа временных рядов $ N $, поскольку они влияют только на частоты.

WEASEL  использует критерий $ \chi^2 $ для выявления наиболее релевантных признаков в каждом классе. Этот критерий определяет значимо ли отличается от наблюдаемая частота от ожидаемой при условии, что данные номинальные. Мы оставляем признаки, для которых $ \chi^2 $ превышает некоторый порог. В среднем этот подход снижает размерность пространства признаков до $ 10^4 \ldots 10^5 $.
...
Можно посмотреть <<остатки>> обученой модели (\pic{fig:residuals_darts})
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
Анализ остатков (\pic{fig:residuals_darts}) показывает, что распределение не центрировано на 0, а значит наша модель \emph{смещена}. Еще можно заметить большие значения автокорреляционной функции на 12-ом лаге. Это означает, что остатки содержат информацию, не учтенную моделью.
...
Смотрим остатки (\pic{fig:residuals_darts_es})
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
Как можно видеть на \pic{fig:residuals_darts_es} распределение остатков стало центрированным, а значения автокоррелционной функции меньше (хотя их нельзя считать незначимыми).
...
Darts поддерживает два вида ковариатов (\pic{fig:covariates}):
\begin{itemize}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=1.25]{figures/covariates.png}
	\caption{ Ковариаты }\label{fig:covariates}
\end{figure}

Каждый ковариат может быть многомерным. Если есть несколько ковариатов, то их можно объединить с помощью \verb|stack()| или \verb|concatenate()| в многомерный временной ряд
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
Не все модели поддерживают все типы ковариатов. Например, \verb|NBEATSModel| поддерживает только \verb|past_covariates|. Для обучения модели нужно просто передать ковариаты методу \verb|fit()|
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
\begin{itemize}
	\item \emph{Многомерные} временные ряды содержат несколько измерений (то есть несколько значений на один временной шаг); еще можно сказать, что многомерный временной ряд это пакет временных рядов, то есть несколько временных рядов, наложенных один на другой (\pic{fig:multivar}).
...
Кроме того, некоторые модели могут работать с \emph{множественными} (multiple) временными рядами, которые представляют собой последовательность нескольких объектов \verb|TimeSeries| -- например, простой список объектов \verb|TimeSeries| (\pic{fig:multiple}). Множественные временные ряды еще называют \emph{панельными данными}. Такие временные ряды не обязаны разделять один и тот же временной индекс. Например, один временной ряд может начинаться с 1990 года, а другой -- с 2000. Они даже не обязаны иметь одну и ту же частоту. 
...
Пример работы с многомерными временными рядами (\pic{fig:multivar_series})
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
\begin{itemize}
	\item Если \verb|n <= output_chunk_length|: мы можем предсказать \verb|n| за один раз (используя один <<внутренний вызов модели>>); в данном примере \verb|n = 1| (\pic{fig:n_lt_output}),
	...
	В данном примере \verb|n = 3|. То есть чтобы построить прогноз, мы должны предоставить следующие \verb|n - output_chunk_length = 2| временные метки (дня). К сожалению, у нас нет измеренных значений температуры в будущем, но если у нас есть прогноз температуры на следующие 2 дня, то можно эти прогнозные точки учесть (\pic{fig:n_gt_output})
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
Если модель обучается с использованием \verb|past_covariates|, то придется эти ковариаты \verb|past_covariates| передать и на шаге построения прогноза. То же самое касается и \verb|future_covariates| с тем отличием, что ковариаты прошлого времени должны иметь по крайней мере те же временные метки, что и у целевого временного ряда, а ковариаты будущего времени должны иметь те же метки, что и у целевого временого ряда + \verb|n| шагов в будущее (\pic{fig:past_future_covariates}).
...
\subsubsection{Панельные временные ряды, предобученные модели и ковариаты}
...
Модели, поддерживающие ковариаты прошлого времени следуют <<блочной>> архитектуре. В основе этих моделей лежит нейронная сеть, которая принимает фрагмент (chunk) временного ряда как вход, и возвращает фрагменты прогнозов (outputs chunks) (\pic{fig:block_arch}). 
...
\paragraph{Обучение модели на панельных временных рядах} При обучении на панельных временных рядах (\pic{fig:panel_ts}) следует имееть в виду, что:
\begin{itemize}
...
Как отмечалось выше, глобальные модели в Darts поддерживают ковариаты. Вот простой пример
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
В данном случае ковариаты (месяц и год) представляют собой многомерные временные ряды. Обучение с ковариатами
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
Построение прогноза с ковариатами
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
Обратное тестирование (backtesting) с ковариатами. Пусть требуется построить прогноз на 12 месяцев, начиная с 60\% точек временного ряда
\begin{lstlisting}[
style = ironpython,
numbers = none
]
backtest_pastcov = model_pastcov.historical_forecasts(
	series_air_scaled,  # временной ряд целиком
	past_covariates=air_covariates,  # ковариаты целиком
	start=0.6,
	forecast_horizon=12,
	stride=1,
	retrain=False,
...
\paragraph{Обучение и прогнозирование многомерных временных рядов} Пример энергопотребления
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
Разбиваем на обучающий и валидационный поднаборы, приводим к диапазону от 0 до 1 и строим прогноз
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
Для ускорения вычислений можно выставить \verb|n_jobjs=-1|
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
\subsubsection{Статические ковариаты}

Модели Darts будут учитывать только статические ковариаты, привязанные к целевому временному ряду. Определить статические ковариаты можно так
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
Добавить статические ковариаты к существующему \verb|TimeSeries| можно так
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
Статические ковариаты можно добавить и при создании экземпляра \verb|TimeSeries|
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
Статические ковариаты полезны только, если мы их используем с несколькими временными рядами
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
Для загрузки наборов предлагаются следующие вспомогательные функции
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
Для упрощения вычисления прогнозов напишем функцию
\begin{lstlisting}[
style = ironpython,
numbers = none
]
def eval_local_model(
	...
	**kwargs
	...
	for series in tqdm(train_series):
		...
		model.fit(series)  # обучается на фрагментах временного ряда
...
Обычно рекомендуется в качестве первого шага посмотреть как себя ведет очень простая модель, просто повторяющая последнее значение
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
Затем можно выяснить какие оценки даст ARIMA, например
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
Теперь можно обратиться к <<\emph{глобальным моделям}>>, которые обучаются на последовательности временных рядов \emph{за один раз}. Darts поддерживает два вида глобальных моделей:
\begin{itemize}
	\item \verb|RegressionModels|, которые представляют собой обертки,
	
	\item Модели на базе PyTorch, которые предлагают различные модели глубокого обучения.
\end{itemize}

Обе группы моделей могут обучаться на нескольких временных рядах путем <<табуляризации>> данных. Вспомогательная функция
\begin{lstlisting}[
style = ironpython,
numbers = none
]
def eval_global_model(
	...
	**kwargs
	...
	model = model_cls(**kwargs)
	model.fit(train_series)  # обучается на всей последовательности разом
...
\verb|RegressionModel| в Darts это прогностические модели, которые могут быть обернуты над любой совместимой с Scikit-learn регрессионной моделью. По сравнению с глубокими моделями, они имеют значительно меньше параметров и могут обучаться гораздо быстрее.

Важные параметры \verb|lags| и \verb|output_chunk_length|. Например \verb|lags=24| и \verb|output_chunk_length=12| означают, что модель будет использовать последние 24 лага, чтобы предсказать следующие 12. Для примера рассмотрим
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
Тепеь рассмотрим глубокие модели
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
Теперь построим, обучим глубокую модель и сделаем прогноз
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
{\color{blue}Линейные модели \verb|LInearRegressionModel| часто дают неплохое решение!} Глубокие модели часто работают гораздо лучше на больших наборах данных. Попробуем тогда глубокие модели на наборе с 48 000 фрагментами. Учитывая, что в среднейм каждый фрагмент содержит 200 временных шагов, то в итоге мы получим 10 млн. обучающих экземпляров
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
При таком количестве экземпляров эпоха занимала бы слишком много времени. Поэтому приходится ограничиваться небольшим числом экземпляров на один фрагмент \verb|max_samples_per_ts|
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
Теперь модель, обченную на наборе M4, можно применить к другому набору (air) -- \emph{трансферное обучение}
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
Аналогичным образом можно обучить и другие глобальные модели и построить прогноз для набора air
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
\subsubsection{Подбор гиперпараметров. Задача прогнозирования электрической нагрузки}

Набор данных \verb|darts.datasets.ElectricityDataset| содержит измерения потребления электроэнергии 370 пользователями (с 15-минутной частотой) (\pic{fig:electricity_dataset}).

Оставим в расмотрении только последние 80 дней в каждом фрагменте
...
Начнем с простой линейной модели без подбора гиперпараметров. Это будет baseline.

ВАЖНО! Получение результатов, превосходящих результаты линейной модели, это не всегда тривиальная задача. Поэтому рекомендуется всегда начинать с простой модели.

\begin{lstlisting}[
style = ironpython,
numbers = none
]
# здесь 7 * DAY_DURATION это ширина окна
lr_model = LinearRegressionModel(lags=7 * DAY_DURATION)  # отступили на 7 дней в прошлое
lr_model.fit(train);
\end{lstlisting}

Теперь построим простую модель TCN
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
"""
Best value: 14.720555851487694, Best params: {
	'days_in': 14, 
	days_out': 6,
	'kernel_size': 19,
	'num_filters': 19,
	'weight_norm': True,
	'dilation_base': 4,
	'dropout': 0.07718156729165897,
	'lr': 0.0008841998396117885,
	'dayofweek': False
}
"""
\end{lstlisting}

Получив наилучшую комбинацию параметров, обучим модель еще раз, но этот раз это будет вероятностная моель
\begin{lstlisting}[
style = ironpython,
numbers = none
]
best_model = build_fit_tcn_model(
	...
	dropout=0.0772,
	lr=0.0008842,
	likelihood=GaussianLikelihood(),  # <- NB
...
Теперь посмотрим как ведет себя модель на тестовом поднаборе
\begin{lstlisting}[
style = ironpython,
numbers = none	
]
...
Исходный временной ряд разбиваем на обучающий и валидационный фрагменты. Приводим ординаты к диапазону от 0 до 1. И строим ковариаты
\begin{lstlisting}[
style = ironpython,
numbers = none	
]
...
Обучаем рекуррентную нейронную сеть с LSTM ячейками
\begin{lstlisting}[
style = ironpython,
numbers = none
]
my_model = RNNModel(
	...
	force_reset=True,
	save_checkpoints=True,  # NB
)
\end{lstlisting}

При обучении модели можно передвать полный ряд ковариат. Модель будет использовать только нужные ей фрагменты
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
Обратное тестирование (backtesting) с помощью обученной \verb|RNNModel|
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
Теперь рассмотрим другой временной ряд \verb|SunspotDataset|
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
series_sunspot.plot()
# найдем лаг сезонности
check_seasonality(series_sunspot, max_lag=240)  # (True, 125)
\end{lstlisting}

Получается приблизительно 11-летняя сезонность. Далее как обычно разбиваем на обучение и валидацию, строим модель
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
my_model_sun = BlockRNNModel(
	model="GRU",
	input_chunk_length=125,  # так как сезонность 125
...
my_model_sun.fit(
    ...
    val_series=val_sp_transformed,
    verbose=True
)
\end{lstlisting}

И снова обратное тестирование
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
\subsubsection{Временная сверточная сеть}

Загружаем набор данных, приводим к диапазону от 0 до 1
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
month_series = datetime_attribute_timeseries(
    ts,
    attribute="month",
    one_hot=True  # NB: временной ряд будет содержать 12 компонент
)
scaler_month = Scaler()
...
Если не задавать число слоев \verb|num_layers|, то оно будет определяться автоматически
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
# Training loss: 0.0016, validation loss: 0.0037, best val loss: 0.0037
\end{lstlisting}
...
\subsubsection{Фильтрация и прогнозирование с использованием фильтров}
...
Гауссовские процессы фильтрации (Gaussian Process filter) похожи на фильтр Калмана (это тоже модель фильтрации, а не прогнозирования). \verb|FilteringModels| используются для \emph{сгалживания} временных рядов. В случае Гауссовсого процесса информация о логике фильтрации закладывается в модель с помощью ядер (kernels).

Для простоты рассмотрим ядро \verb|sklearn.gaussian_process.kernels.ExpSineSquared|. Это ядро кодируется информацию о периодичности. 

Рекомендации по настройке ядер Гауссовских процессов можно найти здесь \url{https://www.cs.toronto.edu/~duvenaud/cookbook/}.

Здесь мы относительную дисперсию шума полагаем \verb|alpha=0.2]|, потому что точно знаем сколько шума мы добавили в сигнал, но на практике эту величину придется оценивать. На практике потребуется очень много экспериментов с типами ядер и уровнем шума
\begin{lstlisting}[
style = ironpython,
numbers = none	
]
...
Гауссовские процессы это \emph{вероятностная модель}, а это означает, что предсказания модели представляют собой распределения сигнала, а не отдельные значения.

Построим 90\%-ые доверительные интервалы
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
Гауссовские процессы работают и на временных рядах с \emph{пропущенными} значениями
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
\begin{figure}[h]
	...
	\caption{ Стратегия LightGBM выращивания деревьев по листьям (leaf-wise). Получаются несиммертичные деревья }\label{fig:lgb_leaf_wise}
\end{figure}
...
Если не задать обратный вызов ранней остановки, то LightGBM отработает все заданные итерации градиентного бустинга (\verb|num_boost_round|). Если же указать раннюю остановку, то обучение остановится на итерации, после которой метрика качества на валидационном поднаборе ухудшается (\pic{fig:lgb_early_stopping}). Метод \verb|lightgbm.cv()| возвращает историю -- значения метрики качества, усредненной по фолдам, как функции числа итераций градиентного бустинга
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
params = {
    "objective": "binary",
    # это имя будет использоваться в качестве ключа в результатах перекрестной проверки
    "metric": METRIC,  
    "verbosity": -1,
}
...
# забираем последнее значение в истории, так как используется ранняя остановка
...
\subsection{Подбор гиперпараметров для LightGBM с помощью Optuna LightGBMTuner и LightGBMTunerCV}
...
\subsection{Шаблон подбора гиперпараметров с помощью связки <<Optuna и LightGBM>>}

Класс целевой функции (функции, которую нужно оптимизировать) выглядит так
\begin{lstlisting}[
style = ironpython,
numbers = none
]
class Objective:
	def __init__(
		...
		self.dtrain = lgb.Dataset(self.X_train, self.y_train)
		self.nfold = nfold
	...
	def __call__(self, trial):                        
		self.params.update({
			...
			"bagging_fraction": 1.0, # trial.suggest_float("bagging_fraction", 0.5, 1.0),
			# "bagging_freq": trial.suggest_int("bagging_freq", 1, 7),
			# "min_child_samples": trial.suggest_int("min_child_samples", 3, 10),
			"min_sum_hessian_in_leaf": trial.suggest_float("min_sum_hessian_in_leaf", 2.0, 10.0),
			"min_gain_to_split": trial.suggest_float("min_gain_to_split", 0.0, 10.0),
		})

		if self.params["boosting_type"] == "dart":
			params["drop_rate"] = trial.suggest_float("drop_rate", 1e-8, 1.0, log=True)
			params["skip_drop"] = trial.suggest_float("skip_rate", 1e-8, 1.0, log=True)
			
		pruning_callback = LightGBMPruningCallback(trial, self.metric)

        # метод возвращает историю
		history = lgb.cv(
			params=self.params,
			...
			nfold=self.nfold,
			callbacks=[
			    lgb.early_stopping(25),  # и ранняя остановка
			    pruning_callback,  # и пранер
			],
			seed=self.seed,
		)

		val_mean_auc = history.get(f"{self.metric}-mean")[-1]

		return val_mean_auc
\end{lstlisting}

То есть откликом целевой функции будет последнее значение усредненной по фолдам метрики качества как функции числа итераций градиентного бустинга -- \verb|history.get("auc-mean")[-1]|. Нас интересует последнее значение, так как в данном случае работает раняя остановка, отсекающая итерации, на которых метрика валдиационного поднабора не улучшалась.

Задаем словарь базовых параметров модели
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
Создаем экземпляр сессии поиска гиперпараметров и запускаем процедуру
\begin{lstlisting}[
style = ironpython,
numbers = none
]
study = optuna.create_study(
	direction="maximize",
	study_name="lgb_custom_tune",
	storage="sqlite:///lgb_custom_tune.db",
	load_if_exists=True,
)
obj = Objective(
	params=params,
...
study.optimize(obj, n_trials=10_000, timeout=3600, gc_after_trial=True, show_progress_bar=True)
"""
Early stopping is not available in dart mode
[I 2023-07-12 20:42:55,774] Trial 2 finished with value: 0.7915843034960872 and parameters: {'boosting_type': 'dart', 'drop_rate': 0.03858299934326663, 'feature_fraction': 0.6576417985829417, 'lambda_l1': 2.0847400336875205e-06, 'lambda_l2': 1.5601726363960928e-07, 'learning_rate': 0.053089739710954674, 'max_depth': 7, 'min_gain_to_split': 9.664951925856915, 'min_sum_hessian_in_leaf': 5.225144389090382, 'num_leaves': 28, 'skip_rate': 0.4467650076723885}. Best is trial 2 with value: 0.7915843034960872.
Training until validation scores don't improve for 25 rounds
Early stopping, best iteration is:
[63]	cv_agg's auc: 0.789041 + 0.0112572
[I 2023-07-12 20:43:00,285] Trial 3 finished with value: 0.7890412948420998 and parameters: {'boosting_type': 'gbdt', 'feature_fraction': 0.6643943025402321, 'lambda_l1': 8.951970272256474e-08, 'lambda_l2': 2.378248488247994e-06, 'learning_rate': 1.5117631876584775e-05, 'max_depth': 10, 'min_gain_to_split': 1.5035164684701863, 'min_sum_hessian_in_leaf': 7.405461764382568, 'num_leaves': 27}. Best is trial 2 with value: 0.7915843034960872.
"""
\end{lstlisting}

Лучшие значения запусков и номер триалов можно получить так
\begin{lstlisting}[
style = ironpython,
numbers = none
]
studyl.best_trial.values  # [0.8101344825181584]
study.best_trial.number  # 97
\end{lstlisting}

А лучшую комбинацию гиперпараметров так
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
"""
{'boosting_type': 'dart',
	'drop_rate': 2.941880473827647e-08,
	'feature_fraction': 0.5055628152323044,
	'lambda_l1': 7.954225765575002e-08,
	'lambda_l2': 0.00047227364783597475,
	'learning_rate': 0.07613335252187517,
	'max_depth': 10,
	'min_gain_to_split': 0.5900059747673241,
	'min_sum_hessian_in_leaf': 5.558746874238254,
	'num_leaves': 80,
	'skip_rate': 6.643407036038301e-05}
"""
\end{lstlisting}

Обучаем модель экстримального градиентного бустинга с наилучшей комбинацией гиперпараметров на всем обучающем наборе данных и забираем последнее значение метрики качества
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
"""
...
[993]	training's auc: 0.873883	valid_1's auc: 0.820188
[994]	training's auc: 0.873883	valid_1's auc: 0.820188
[995]	training's auc: 0.873883	valid_1's auc: 0.820188
[996]	training's auc: 0.873883	valid_1's auc: 0.820188
[997]	training's auc: 0.873883	valid_1's auc: 0.820188
[998]	training's auc: 0.873883	valid_1's auc: 0.820188
[999]	training's auc: 0.873883	valid_1's auc: 0.820188
[1000]	training's auc: 0.873883	valid_1's auc: 0.820188
"""
\end{lstlisting}

ВАЖНО: ранняя остановка не доступна, когда используются DART-деревья.

Еще раз. После такого как наилучшая комбинация гиперпараметров найдена, модель обучается на полном обучающем наборе данных \verb|(X_sub_train, y_sub_train)| с наилучшей комбинацией гиперепараметров, найденной с помощью перекрестной проверки, и оценивает метрику качества на внешнем валидационном поднаборе \verb|(X_val,)| (для того чтобы можно было корректно сравнивать разные модели, модели сравниваются именно на валидационном поднаборе) для каждой итерации градиентного бустинга. 

Получить лучшие значения метрики можно следующим образом
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
defaultdict(collections.OrderedDict,
	{'training': OrderedDict([('auc', 0.8738825455237967)]),
		'valid_1': OrderedDict([('auc', 0.820187936036874)])})
"""
\end{lstlisting}
...
roc_auc_score(y_sub_train, model.predict(X_sub_train))  # 0.8738825455237967 
roc_auc_score(y_val, model.predict(X_val))  # 0.8201879360368739
\end{lstlisting}
...
\subsection{Визуализация с optuna-dashboard}

Для того чтобы получить dashboard результатов подбора гиперпараметров в режиме реального времени, достаточно просто установить \verb|pip install optuna-dashboard|.

При создании сессии подбора гиперпарметров нужно указать путь до базы данных с помощью параметра \verb|storage|
\begin{lstlisting}[
style = ironpython,
numbers = none
]
study = optuna.create_study(
    direction="maximize",
    study_name="lgb_custom_tune",
    storage="sqlite:///lgb_custom_tune.db",  # << NB
    load_if_exists=True,
)
...
По завершении поиска нужно вызывать optuna-dashboard (хотя можно и параллельно с поиском; dashboard будет обновляться в режиме близком к реальному времени)
\begin{lstlisting}[
style = ironpython,
numbers = none
]
$ optuna-dashboard sqlite:///lgb_custom_tune.db
\end{lstlisting}

На локальном петлевом интерфейсе \verb|http://localhost:8080/| будет доступен dashboard.
...
Как отмечается в документации технику Zero-Shot AutoML еще можно использовать, если при обучении модели задать \verb|starting_points="data"|, \verb|max_iter=0| и \verb|time_budget=None|
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
Этот вариант с \verb|AutoML()| будет вести себя также как \verb|default.LGBMClassifier()|.

При сохранении модели нужно вызвать бустер
\begin{lstlisting}[
style = ironpython,
numbers = none
]
joblib.dump(clf_zero_shot.booster_, "./clf_zero_shot_lgbm.pkl")
booster = joblib.load("./clf_zero_shot_lgbm.pkl")  # <lightgbm.basic.Booster at 0x1ba689a15e0>
booster.predict(X_val)  # array([0.29226837, 0.00657792, 0.54021481, ..., 
\end{lstlisting}

ВАЖНО! Метод \verb|predict| бустера возвращает вероятность принадлежности экземпляра положенительному классу, а метод \verb|predict| модели\\ \verb|flaml.default.estimator.flamlize_estimator.<locals>.EstimatorClass| -- метки класса. То есть чтобы получить вероятности нужно делать так
\begin{lstlisting}[
style = ironpython,
numbers = none	
]
clf_zero_shot.booster_.predict(X_val)  # array([0.29226837, 0.00657792, 0.54021481, ...
clf_zero_shot.predict_proba(X_val)[:, 1]  # array([0.29226837, 0.00657792, 0.54021481, ...
\end{lstlisting}
