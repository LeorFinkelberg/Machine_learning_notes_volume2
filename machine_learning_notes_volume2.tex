...
Теперь обучаем Zero-shot AutoML FLAML и вычисляем метрику качества на валидационном поднаборе данных. Метрика вычисляется на том же самом поднаборе данных и потому можно корректно сравнивать эти две модели.

В случае Optuna LightGBMTunerCV модель обучается не на том же наборе данных, что и предыдущие модели. <<За кадром>> обучающий набор разбивается на подобучающие и валидационные поднаборы (а у нас еще есть другой валидационный набор, на котором мы сравниваем модели!) и таким образом модель обучается на обучающем наборе меньшего размера, а гиперпараметры подбираются, так сказать, на внутреннем валидационном поднаборе, а не на внешнем. После подбора гиперпараметров можно вычислить метрику качества на внешнем валидационном поднаборе и корректно сравнить модели.
